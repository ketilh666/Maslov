...
subroutine write_debug(ishot,file_ascii,file_npat,             &
   ...
   integer   :: nhor, kth(10), jh, kh, lewis
   real(kr4) :: rhor(0:3)
...
!   lua = iku_get_file_unit(file_ascii)
   lua = 6 ! Send this crap to stdout
   lewis = iku_get_file_unit(file_ascii)
...
!CUT   write(lewis,*) 'KH: ishot = ',ishot
   irec = irec0
   do ir=1,ksex%nray,inc
      do k=1,ksex%nel(ir)-1
!      do k=1,ksex%maxel
         irec = irec+1
         ray_xz(1) = real(ksex%xray(3,k,ir))       ! xz plane: z
         ray_xz(2) = real(ksex%xray(1,k,ir))       ! xz plane: x
         ray_yz(1) = real(ksex%xray(3,k,ir))       ! yz plane: z
         ray_yz(2) = real(ksex%xray(2,k,ir))       ! yz plane: y
         call iku_write(file_ray_xz,ray_xz)
         call iku_write(file_ray_yz,ray_yz)
!CUT         rhor(0) = 1000*real(ksex%time(  k,ir))  ! Time in ms    
!CUT         rhor(1) = real(ksex%xray(1,k,ir))       
!CUT         rhor(2) = real(ksex%xray(2,k,ir))       
!CUT         rhor(3) = real(ksex%xray(3,k,ir))
!CUT         write(lewis,7) rhor(:)       
      end do
   end do
   irec0 = irec
...
!   write(lua,5) '                       nout_cx = ',nout_cx
   ...
   write(  6,5) 'Total number of rays:     nray = ',ksex%nray
   write(  6,6) 'Interpolation #1:     npa,naz2 = ',ksex%npol,ksex%nazi(2)
   ...
   do jh=1,nhor   
      ...
      !write(lewis,*) '----------------------------------------------'
      !write(lewis,5) ' Isotherm: ',jh
      !write(lewis,*) '----------------------------------------------'
  
      kh = kth(jh)
      do ir=1,ksex%nray
         rhor(0) = real(ksex%time(  kh,ir))  ! Time in ms    
         rhor(1) = real(ksex%xray(1,kh,ir))       
         rhor(2) = real(ksex%xray(2,kh,ir))       
         rhor(3) = real(ksex%xray(3,kh,ir))
         write(lewis,7) rhor(:)       
      end do
