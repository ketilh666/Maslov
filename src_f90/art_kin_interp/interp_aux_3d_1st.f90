...
subroutine three_edges(isort,vhead,vtail,vertx,nc,ned)
   ...
   !----------------------------------------------------
   !  Head and tail of edge vectors
   !----------------------------------------------------

   vtail(1:nc,1) = vertx(1:nc,1)
   vtail(1:nc,2) = vertx(1:nc,2)
   vtail(1:nc,3) = vertx(1:nc,3)
   vhead(1:nc,1) = vertx(1:nc,2)
   vhead(1:nc,2) = vertx(1:nc,3)
   vhead(1:nc,3) = vertx(1:nc,1)
   ...
   do ie = 1,ned
      if (vhead(2,ie).lt.vtail(2,ie)) then
         wrk2(1:nc)     = vhead(1:nc,ie)
         vhead(1:nc,ie) = vtail(1:nc,ie)
         vtail(1:nc,ie) = wrk2(1:nc)
      end if
   enddo
   
   !----------------------------------------------------
   !   Sort edge-tails in ascending order on y-coord.
   !----------------------------------------------------

   wrk3(1:ned) = real(vtail(2,1:ned),kind=kr4)
   call SHELLA(ned,wrk3,isort)

   !----------------------------------------------------
   !   Chech head of vectors 1 and 2
   !----------------------------------------------------

   ie1 = isort(1)
   ie2 = isort(2)

   if (vhead(2,ie1).gt.vhead(2,ie2)) then
      isort(1) = ie2
      isort(2) = ie1
   endif
...
!********************************************************************* 
!
!  IKU Seismic subroutine junk_edge
!
!  Purpose : Junk active edge if the current y-coordinate is
...
subroutine junk_edge(ie1,ie2,isort,ned)

   implicit none

   !--- External variables: 
   integer   ,intent(inout) :: ie1,ie2     ! Active edges
   integer   ,intent(in)    :: isort(ned)  ! Sort index array
   integer   ,intent(in)    :: ned      

   !--- Internal variables: 

   !----------------------------------------------------
   !  Junk first active edge
   !----------------------------------------------------
