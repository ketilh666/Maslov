...
C
C NOTE:    This routine is what we used first. 
C          I think it is wrong.
C
...
C************************************************************************
      ...
      SUBROUTINE BILL(QXDOT,PXDOT,QX,PX,DGAMDX,DGAMDP,DGAMXX,
...
C---              2nd term:
                  ...
                  H2PX = H2PX + DGAMDP(I,J,K)*GIDGJX(I,J,L)
                  H2XP = H2XP + DGAMDX(I,J,K)*GIDGJP(I,J,L)
               ENDDO
            ENDDO
...
C-----------------------------------------------------------------------
      ...
      DO K=1,2
         DO J=1,3
            ...
            PXDOT(J,K) = 0.0
            DO I=1,3
               QXDOT(J,K) = QXDOT(J,K) + DHDPX(J,I)*QX(I,K)
     +                                 + DHDPP(J,I)*PX(I,K)
               PXDOT(J,K) = PXDOT(J,K) - DHDXP(J,I)*PX(I,K)
     +                                 - DHDXX(J,I)*QX(I,K)
            ENDDO
         ENDDO
      ENDDO
