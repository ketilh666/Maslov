...
C SUBROUTINE MASI2
C
C PURPOSE: Compute the 3D Maslov integral for I={1,2}
C
C REFERENCES: 
...
C************************************************************************
      ...
      SUBROUTINE MASI2(CW,CMI,TRAY,XRAY,PRAY,VGRAY,GVRAY,Q2RAY,P2RAY,
...
C---           Maslov Phase:
               ...
               TMAS = T1 + P1*X1+P2*X2
               CARG = CI*(CW*CMPLX(TMAS) + 0.5*4*PI*(CW/ABS(CW)))
